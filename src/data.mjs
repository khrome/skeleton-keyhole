...
export class KeyholeDataLayer extends HTMLElement{
    constructor(){
        super();
    }
    getKeyhole(){
    ...
    getWrappedData(){ //ensure that a list is wrapped as a collection
    ...
    connectedCallback(){
        var event = new Event('keyhole-dom-data-add', {bubbles: true});
        event.el = this;
    ...
    mapAttach(map, engine){
        ...
        this.loaded = true;
        var event = new Event('keyhole-map-data-add', {bubbles: true});
        event.el = this;
        event.detail = { name, data };
        this.dispatchEvent(event);
    }
